...
def overlay():
    ...
    if request.method == 'GET':
        receivedIP = receivedData["ip"]
        receivedPort = receivedData["port"]
        receivedOverlay = json.loads(receivedData["overlay"])
        receivedOverlay.append(inf.myPrivateIP+":"+inf.myPort)

        if receivedIP+":"+receivedPort == nextIP+":"+nextPort:
            pool.apply_async(req.post, ["http://" + receivedIP + ":" + receivedPort + '/overlay'],
                             {"data": {"data": json.dumps(receivedOverlay)}})
        else:
            pool.apply_async(req.get, ["http://" + nextIP + ":" + nextPort + '/overlay'],
                             {"data": {"ip": receivedIP, "port": receivedPort, "overlay": json.dumps(receivedOverlay)}})

        return {"respons": "Message Forwarded"}

    elif request.method == 'POST':

        receivedData = json.loads(request.form.to_dict()["data"])
...
if __name__ == '__main__':
    pool = mp.Pool(5)
    t1 = threading.Thread(target=ca.application)
    t1.start()
